...
if __name__ == "__main__":
    ...
    for i in range(13,21):
        fens = np.load("datasets/fen_list"+str(i)+".npy")
        x_temp = []
        y_temp = []
        j = 0
        for f in fens:
            j+=1
            if j%1000==0:
                print("fen", str(i), "puzzle", str(j))
                print("fails:", fails)

            b = chess.Board(fen=f)


            info = engine.analyse(b, chess.engine.Limit(depth=6))
            try:
                uci = info['pv'][0].uci()
            except:
                fails+=1
                continue
            x_temp.append(encode_board(b))
            start = (ord(uci[0])-97) * 8 + int(uci[1]) -1
            end = (ord(uci[2])-97) * 8 + int(uci[3]) -1
            y_temp.append(start * 64 + end)
        ...
        np.save("datasets/flx_train_moves"+str(i), x_train)
        np.save("datasets/fly_train_moves"+str(i), y_train)
    engine.quit()
